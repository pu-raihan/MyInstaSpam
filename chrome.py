...
def login(usr, pwd):
    ...
    # goto target page
    chrome.get('https://instagram.com/'+ target )
    time.sleep(4)
    
    if q == 'y':
        return
    else:
        chrome.find_element_by_class_name('_6VtSN').click()
        time.sleep(3)
...
print ('{0}>> {1}Initializing chrome driver...'.format(RED,GREEN,END))
path()
time.sleep(1)
url_name(url)
...
if q == 'n':
    chrome.get('https://instagram.com/'+ target )
    chrome.find_element_by_class_name('-fzfL').click()
    time.sleep(2)
    chrome.find_element_by_class_name('-Cab_').click()
    time.sleep(2)
